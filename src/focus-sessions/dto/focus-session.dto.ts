...
export class CreateFocusSessionDto {
  ...
  @ApiProperty({ example: 1, description: 'ID of the emotion before starting the session' })
  emotionBeforeId: number;
}

// finish-focus-session.dto.ts
export class FinishFocusSessionDto {
  @ApiProperty({ example: 2, description: 'ID of the emotion after finishing the session' })
  emotionAfterId: number;
}

// cancel-focus-session.dto.ts
  export class CancelFocusSessionDto {
  @ApiProperty({ example: 8, description: 'Minutes actually focused before cancel' })
    @IsInt()
    @Min(0)
